...
const macOSArchievePath = 'build/macos/Build/Products/Release/Nahida.app';
const releaseDir = '/Volumes/pm981/release/Nahida';
...
Future<void> flutterBuildLinux() async {
  ...
  // mkdir Nahida.AppDir
  await Process.run('mkdir', ['Nahida.AppDir']);
  // cp -r build/linux/x64/release/bundle/* Nahida.AppDir
  await Process.run('cp', [
    '-r',
    './build/linux/x64/release/bundle/*',
    'Nahida.AppDir'，
  ]);
  // cp -r assets/app_icon.png Nahida.AppDir
  await Process.run('cp', [
    '-r',
    './assets/app_icon.png',
    'Nahida.AppDir'，
...
exec ./Nahida
''';
  await File('Nahida.AppDir/AppRun').writeAsString(appRun);
  // chmod +x AppRun
  await Process.run('chmod', ['+x', 'Nahida.AppDir/AppRun']);
  // Create .desktop
  const desktop = '''
[Desktop Entry]
Name=Nahida
...
Categories=Network;
''';
  await File('Nahida.AppDir/Nahida.desktop').writeAsString(desktop);
  // Run appimagetool
  await Process.run('appimagetool', ['Nahida.AppDir']);

  await scpLinux2CDN();

  // Clean build files
  await Process.run('rm', ['-r', 'Nahida.AppDir']);
  await Process.run('rm', ['Nahida-x86_64.AppImage']);
}
...
Future<void> scpApk2CDN() async {
  ...
  final result = await Process.run(
    'scp',
    [apkPath, 'hk:/var/www/res/Nahida/$sha256.apk'],
    runInShell: true,
  );
  if (result.exitCode != 0) {
    print(result.stderr);
...
Future<void> scpLinux2CDN() async {
  final result = await Process.run(
    'scp',
    [
      'Nahida-x86_64.AppImage'，
      'hk:/var/www/res/Nahida/$build.AppImage',
    ],
    runInShell: true,
  );
  if (result.exitCode != 0) {
...
Future<void> scpWindows2CDN() async {
  final result = await Process.run(
    'scp',
    [
      './build/windows/runner/Release/nahida.zip',
      'hk:/var/www/res/Nahida/$build.zip',
    ],
    runInShell: true,
  );
  if (result.exitCode != 0) {
